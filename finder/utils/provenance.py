...
The detection is shared with ``filer.utils.provenance``; ``encode_image`` is specific to
finder, which re-encodes stored originals (EXIF orientation, oversized images).
"""
import io
import re
import struct
...
def encode_image(image, image_format, digital_source_type="", **save_kwargs):
    """Encode a Pillow image and return its bytes. A plain ``save()`` drops all metadata,
    so the digital source type, if given, is written back in a minimal XMP packet. C2PA
    Content Credentials cannot be kept: their signature covers the original bytes."""
    xmp = build_xmp_packet(digital_source_type) if digital_source_type else None
    if xmp and image_format == "PNG":
        ...
        # WebP, AVIF; Pillow < 11 cannot write XMP into a JPEG, nor into a GIF
        save_kwargs["xmp"] = xmp
    buffer = io.BytesIO()
