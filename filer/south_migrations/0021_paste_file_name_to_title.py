...
class Migration(DataMigration):
    """
    Duplicates data from name field to title field.
    """

    def forwards(self, orm):
        File = orm['filer.File']
        files_with_names = File.objects.exclude(
            Q(name__isnull=True) | Q(name__exact='')).values_list('id', 'name')
        for file_id, file_name  in files_with_names:
            File.objects.filter(id=file_id).update(title=file_name)
    ...
    def backwards(self, orm):
        ...
        files_with_names = File.objects.exclude(
            Q(name__isnull=True) | Q(name__exact='')).values_list('id', 'name')
        for file_id, file_name  in files_with_names:
            File.objects.filter(id=file_id).update(title='')
