...
* feat: Thumbnails of public files now honour easy-thumbnails' ``THUMBNAIL_NAMER``
  ...
  using easy-thumbnails' default namer, since ``filer.server.views`` has to be able
  to derive the source file from a thumbnail name in order to check permissions
  on it.

.. warning::

   Thumbnail file names change with this release: ``photo.jpg__100x100_q85.jpg``
   becomes ``photo.jpg.100x100_q85.jpg``. Thumbnails are regenerated on demand, so
   no action is required, but the thumbnails generated by earlier versions stay
