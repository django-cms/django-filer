...
class ServeProtectedThumbnailTests(TestCase):
    ...
    def test_serve_thumbnail_legacy_name_authenticated(self):
        """Thumbnails generated by filer < 3.6 keep working after the upgrade."""
        thumbnailer = self.filer_image.easy_thumbnails_thumbnailer
        thumbnail = thumbnailer.get_thumbnail({'size': (100, 100)})
        storage = filer_settings.FILER_PRIVATEMEDIA_THUMBNAIL_STORAGE
        # Re-save the thumbnail under the naming scheme of filer < 3.6
        source_name = self.filer_image.file.name
        legacy_name = os.path.join(
            thumbnailer.thumbnail_basedir, f'{source_name}__100x100_q85.jpg')
        with storage.open(thumbnail.name) as thumbnail_file:
            legacy_name = storage.save(legacy_name, thumbnail_file)
        self.addCleanup(storage.delete, legacy_name)

        thumb_url = '/' + storage.base_url.lstrip('/') + legacy_name
        response = self.client.get(thumb_url)
        self.assertEqual(
